...
export class UsuariosService {
    ...
    async crearUsuario(data: iCrearUsuario) {
        try {
            const usuario:iUsuario | null = await this.usuariosRepository.consultarUsuario(data.usuario);
            if(usuario != null){
                throw new Error('El nombre de usuario ya se encuentra asignado');
            }else{
    ...
    async consultarUsuario(usuario: string) {
        try {
            const resultado = await this.usuariosRepository.consultarUsuario(usuario);
            return resultado;
        } catch (error: any) {
    ...
    async loginUsuario(credentials: iLogin, ip:string, user_agent:string):Promise<iTokensAcceso> {
        try {
            ...
            const usuario:iUsuario | null = await this.usuariosRepository.consultarUsuario(credentials.usuario);
            if(usuario == null){
